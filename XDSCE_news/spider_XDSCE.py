...
@showresult
def GetHtml(num):
	url = 'http://ce.xidian.edu.cn/info/1020/%s.htm'%str(num)
	print(url)
...
def Update():
	...
	log.close()
	print(history)
	url = 'http://ce.xidian.edu.cn/xwyhd.htm'
	request = urllib.request.Request(url)
	try:
		response = urllib.request.urlopen(request)
		...
		print("无法获得通知列表！")
		return 1
	info_list = re.findall(r'<A href="info/1020/(.*?)\.htm',content)
	...
	for i in info_list:
		if i in history:
			...
			#if pushover.SendWXnews(res["title"],res["time"]+'\n'+res["detail"],url='http://ce.xidian.edu.cn/info/1020/%s.htm'%i):
			url='http://ce.xidian.edu.cn/info/1020/%s.htm'%i
